...
#alps <- data.frame(x = 14.597570957196247, y = 46.36075461658624) %>%
...
elev <- get_elev_raster(alps, z = 12)
...
## phytoplankton data: https://neo.gsfc.nasa.gov/view.php?datasetId=MY1DMM_CHLORA 
...
## Day 11: 3D.
Attempting to plot Ontario (Toronto?) postcodes by population.
...
ggplot() +
  geom_polygon(data = california, aes(x=long, y = lat, group = group ), fill= 'grey80') + 
  #geom_hex(data = whale1, aes(x = decimalLongitude, y = decimalLatitude, fill= stat(log(count))), binwidth = c(.6,.5)) +
#  geom_point(data=whales, aes(x = location.long, y = location.lat, group = tag.local.identifier, color = tag.local.identifier), size=1) +
  geom_point(data = blue_whales, aes(x = argos.lon2, y = argos.lat2, group = tag.local.identifier, color = tag.local.identifier), size=1) +
  geom_path(data = blue_whales[order(blue_whales$timestamp),], aes(x = argos.lon2, y = argos.lat2, group=tag.local.identifier))+
  coord_fixed(1.3) +
  #scale_fill_gradientn(colors=c("paleturquoise4", "#D5BB9F", "#C42E2A"))+
  theme_minimal() 
  #theme(panel.grid.major = element_blank(), panel.grid.minor = element_blank(),
...
shenandoah <- data.frame(x = -78.3174017258264, y = 38.55778630513009) %>%
  st_as_sf(coords = c('x','y'), crs = 4326)

elev <- get_elev_raster(shenandoah, z = 6)
elev <- get_elev_raster(shenandoah, z = 14)

elmat <- raster_to_matrix(elev) 

elmat %>%
  sphere_shade(texture = 'imhof1')%>%
  #sphere_shade(texture = create_texture('lightgreen','darkblue','grey','grey','lightblue')) %>%
  add_shadow(lamb_shade(elmat)) %>%
  plot_3d(elmat, zscale=10, fov = 0, theta = 60, zoom = 0.7, phi = 65, windowsize = c(1000, 800))
  #add_shadow(ray_shade(nh_elmat, sunaltitude=20)) %>%
  #plot_map()
